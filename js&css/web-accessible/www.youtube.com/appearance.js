        ...
        if (window.yt.config_.EXPERIMENT_FLAGS.kevlar_watch_grid !== false) {
          try {
            newSidebarFlags.forEach((F) => {
              Object.defineProperty(window.yt.config_.EXPERIMENT_FLAGS, F, {
                get: () => false,
              });
            });
          } catch (error) {
...
ImprovedTube.forcedTheaterMode = function () {
  if (
    ...
    var button = ImprovedTube.elements.player.querySelector(
...
ImprovedTube.playerHdThumbnail = function () {
  if (this.storage.player_hd_thumbnail === true) {
    ...
    if (thumbnail.style.backgroundImage.indexOf("/hqdefault.jpg") !== -1) {
      thumbnail.style.backgroundImage = thumbnail.style.backgroundImage.replace(
        "/hqdefault.jpg",
        "/maxresdefault.jpg"
      );
    }
  ...
  if (player && player.className.indexOf("ytp-autohide") !== -1) {
    ...
    for (let i = 0, l = play_bars.length; i < l; i++) {
      ...
      if (played - progress_play >= a) {
        ...
        b =
          (100 * ((played - progress_play) * width_percent)) /
          play_bars[i].offsetWidth;
      }
      ...
      if (loaded - progress_load >= a) {
        ...
        c =
          (100 * ((loaded - progress_load) * width_percent)) /
          play_bars[i].offsetWidth;
      }
...
ImprovedTube.playerRemainingDuration = function () {
  ...
  let remainingEl = durationEl.parentNode.querySelector(".it-remaining-time");

  if (!remainingEl) {
    remainingEl = document.createElement("span");
    remainingEl.className = "it-remaining-time";
    remainingEl.style.marginLeft = "4px";
    remainingEl.style.opacity = "0.8";
    durationEl.parentNode.appendChild(remainingEl);
  }
  ...
  if (ImprovedTube.storage.comments_sidebar_simple === true) {
    if (window.matchMedia("(min-width: 1599px)").matches) {
      document
        ...
        .insertAdjacentElement("afterend", document.querySelector("#comments"));
    }
  ...
  if (ImprovedTube.storage.comments_sidebar === true) {
    ...
    function setGrid() {
      let checkParentInterval = setInterval(() => {
        container = document.querySelector(
          "#related ytd-compact-video-renderer.style-scope"
        )?.parentElement;
        if (container) {
    ...
    function styleScrollbars() {
      if (!navigator.userAgent.toLowerCase().includes("mac")) {
        ...
        const isDarkMode =
          getComputedStyle(document.querySelector("ytd-app")).getPropertyValue(
...
ImprovedTube.transcript = function (el) {
  if (ImprovedTube.storage.transcript === true) {
    const available =
      el.querySelector("[target-id*=transcript][visibility*=HIDDEN]") ||
      el.querySelector("[target-id*=transcript]")?.clientHeight;
    if (available) {
      if (!ImprovedTube.originalFocus) {
        ImprovedTube.originalFocus = HTMLElement.prototype.focus;
      ...
      ImprovedTube.forbidFocus = function (ms) {
        HTMLElement.prototype.focus = function () {
          console.log("Preventing YouTube's scripted scrolling for a moment.");
        };
        if (document.hidden) ms = 3 * ms;
        setTimeout(function () {
          HTMLElement.prototype.focus = ImprovedTube.originalFocus;
        }, ms); // Restoring JS's "focus()"
      };
      ImprovedTube.forbidFocus(2100);
      const descriptionTranscript = el.querySelector(
      ...
      if (yt.config_.EXPERIMENT_FLAGS.kevlar_watch_grid === true) {
        available.setAttribute("z-index", "98765");
      }
    }
  }
};
...
ImprovedTube.chapters = function (el) {
  if (ImprovedTube.storage.chapters === true) {
    const available =
      el.querySelector("[target-id*=chapters][visibility*=HIDDEN]") ||
...
ImprovedTube.improvedtubeYoutubeButtonsUnderPlayer = function () {
   ...
   */
    if (section) {
      if (
        this.storage.below_player_loop !== false &&
        !document.querySelector("#it-below-player-loop")
      ) {
        ...
        button.onclick = function () {
          ...
          if (video.hasAttribute("loop")) {
            ...
            !/ad-showing/.test(ImprovedTube.elements.player.className)
          ) {
      ...
      if (
        this.storage.below_player_pip !== false &&
        !document.querySelector("#it-below-player-pip")
      ) {
        var button = document.createElement("button"),
          svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"),
          path = document.createElementNS("http://www.w3.org/2000/svg", "path");
      ...
      if (
        this.storage.below_player_screenshot !== false &&
        !document.querySelector("#it-below-player-screenshot")
      ) {
        var button = document.createElement("button"),
          svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"),
          path = document.createElementNS("http://www.w3.org/2000/svg", "path");
        ...
        path.setAttributeNS(
          null,
          "d",
          "M21 19V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14c0 1.1.9 2 2 2h14a2 2 0 0 0 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"
        );
      ...
      if (
        this.storage.below_player_keyscene !== false &&
        !document.querySelector("#it-below-player-keyscene")
      ) {
        var button = document.createElement("button"),
          svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"),
        ...
        path.setAttributeNS(
          null,
          "d",
          "M13 2 L3 14 H10 L8 22 L20 10 H13 L15 2 Z"
        );
        ...
        if (this.storage.below_player_screenshot !== false) {
          ...
          screenshotButton?.parentElement?.insertBefore(
            button,
            screenshotButton
          );
        } else {
      ...
      if (
        this.storage.copy_video_id === true &&
        !document.querySelector("#it-below-player-copy-video-id")
      ) {
        var button = document.createElement("button"),
          svg = document.createElementNS("http://www.w3.org/2000/svg", "svg"),
          path = document.createElementNS("http://www.w3.org/2000/svg", "path");
    ...
    setTimeout(function () {
      ...
      if (!element) {
        ...
        try {
          ...
          try {
            document
              .querySelector("#info #info-strings yt-formatted-string")
              ?.append(label);
          } catch { }
        }
...
ImprovedTube.exactUploadDate = function () {
  if (this.storage.exact_date === true) {
    ...
    xhr.addEventListener("load", function () {
      ...
      const element =
        ImprovedTube.elements.exact_date || document.createElement("div");

      ImprovedTube.empty(element);

      if (response.error) {
        element.appendChild(
          document.createTextNode(`• Error: ${response.error.code}`)
        );
      } else {
        const publishedDate = new Date(response.items[0].snippet.publishedAt);
        element.appendChild(
          document.createTextNode(`• ${publishedDate.toLocaleDateString()}`)
        );
      }
...
ImprovedTube.howLongAgoTheVideoWasUploaded = function () {
  if (
    this.storage.how_long_ago_the_video_was_uploaded === true &&
      ...
      if (response.error) {
        ...
        element.appendChild(
          document.createTextNode(
            "• " + timeSince(response.items[0].snippet.publishedAt)
          )
        );
      }
...
ImprovedTube.channelVideosCount = function () {
  if (
    this.storage.channel_videos_count === true &&
    this.elements.yt_channel_link
  ) {
    ...
    if (this.elements.yt_channel_link.href.indexOf("/channel/") == -1) {
      ...
      id = this.elements.yt_channel_link.href.slice(
        this.elements.yt_channel_link.href.indexOf("/channel/") +
        "/channel/".length
      );
      if (id.indexOf("/") !== -1) {
        id = id.match(/.+?(?=\/)/)[0];
    ...
    xhr.addEventListener("load", function () {
      var response = JSON.parse(this.responseText),
        parent = document.querySelector(
          "#meta ytd-channel-name + yt-formatted-string"
        ),
        element =
          ImprovedTube.elements.channel_videos_count ||
          document.createElement("div");
      ...
      if (response.error) {
        ...
        element.appendChild(
          document.createTextNode(
            "• " + response.items[0].statistics.videoCount + " videos"
          )
        );
      }
