...
ImprovedTube.toggleFitToWindow = function () {
  ...
  let isFTW =
    document.querySelector("html").getAttribute("it-player-size") ===
    "fit_to_window";
  if (isFTW) {
    ...
    document
      .querySelector("html")
      .setAttribute("it-player-size", "fit_to_window");
  }
...
ImprovedTube.playerCinemaModeButton = function () {
  if (this.storage.player_cinema_mode_button && /watch\?/.test(location.href)) {
    ...
    this.createPlayerButton({
      ...
      opacity: 0.64,
      onclick: function () {
        var player = xpath('//*[@id="movie_player"]/div[1]/video')[0].parentNode
          .parentNode;
        // console.log(player)
...
ImprovedTube.playerPopupButton = function () {
  if (
    this.storage.player_popup_button === true &&
    ...
    path.setAttributeNS(
      null,
      "d",
      "M19 7h-8v6h8V7zm2-4H3C2 3 1 4 1 5v14c0 1 1 2 2 2h18c1 0 2-1 2-2V5c0-1-1-2-2-2zm0 16H3V5h18v14z"
    );
      ...
      onclick: function () {
        ...
        const videoID = location.search.match(ImprovedTube.regex.video_id)[1],
          ...
          popup = window.open(
            `${
              location.protocol
            }//www.youtube.com/embed/${videoID}?start=${parseInt(
              ytPlayer.getCurrentTime()
            )}&autoplay=${
              ImprovedTube.storage.player_autoplay_disable ? "0" : "1"
            }${listMatch ? `&list=${listMatch[1]}` : ""}`,
            "_blank",
            `directories=no,toolbar=no,location=no,menubar=no,status=no,titlebar=no,scrollbars=no,resizable=no,width=${
              ytPlayer.offsetWidth / 3
            },height=${ytPlayer.offsetHeight / 3}`
          );
        if (popup && listMatch) {
          ...
          popup.addEventListener(
            ...
            function () {
              ...
              const videoLink = this.document.querySelector(
                "div#player div.ytp-title-text>a[href]"
              );
              if (
                videoLink &&
                videoLink.href.match(ImprovedTube.regex.video_id)[1] !== videoID
              )
                this.location.search = this.location.search.replace(
                  /(\?)list=[^&]+&|&list=[^&]+/,
                  "$1"
                );
            },
...
CUSTOM MINI-PLAYER
------------------------------------------------------------------------------*/
ImprovedTube.mini_player__setSize = function (
  width,
  height,
  keep_ar,
  keep_area
  ...
  if (keep_ar) {
    const aspect_ratio =
      ImprovedTube.elements.video.style.width.replace("px", "") /
      ImprovedTube.elements.video.style.height.replace("px", "");
    if (keep_area) {
...
ImprovedTube.miniPlayer_scroll = function () {
  if (
    ...
    ImprovedTube.elements.player.classList.contains("ytp-player-minimized") ===
      false
  ) {
    ImprovedTube.mini_player__mode = true;

    ImprovedTube.mini_player__original_width =
      ImprovedTube.elements.player.offsetWidth;
    ImprovedTube.mini_player__original_height =
      ImprovedTube.elements.player.offsetHeight;

    ImprovedTube.elements.player.classList.add("it-mini-player");

    ImprovedTube.mini_player__x = Math.max(
      0,
      Math.min(
        ImprovedTube.mini_player__x,
        document.body.offsetWidth - ImprovedTube.mini_player__width
      )
    );
    ImprovedTube.mini_player__y = Math.max(
      0,
      Math.min(
        ImprovedTube.mini_player__y,
        window.innerHeight - ImprovedTube.mini_player__height
      )
    );
    ...
    ImprovedTube.elements.player.style.transform =
      "translate(" +
      ImprovedTube.mini_player__x +
      "px, " +
      ImprovedTube.mini_player__y +
    ...
    ImprovedTube.mini_player__setSize(
      ImprovedTube.mini_player__width,
      ImprovedTube.mini_player__height,
      true,
      true
    );
    ...
    (window.scrollY < 256 && ImprovedTube.mini_player__mode === true) ||
    ImprovedTube.elements.player.classList.contains("ytp-player-minimized") ===
      true
  ) {
    ImprovedTube.mini_player__mode = false;
    ...
    window.removeEventListener(
      "mousemove",
      ImprovedTube.miniPlayer_cursorUpdate
    );
  }
};
...
ImprovedTube.miniPlayer_mouseDown = function (event) {
  ...
  for (var i = 0, l = path.length; i < l; i++) {
    if (
      (path[i].classList && path[i].classList.contains("it-mini-player")) ===
      true
    ) {
      is_player = true;
    }
  ...
  ImprovedTube.mini_player__max_x =
    document.body.offsetWidth - ImprovedTube.mini_player__width;
  ImprovedTube.mini_player__max_y =
    window.innerHeight - ImprovedTube.mini_player__height;
...
ImprovedTube.miniPlayer_cursorUpdate = function (event) {
  ...
  if (
    x >=
      ImprovedTube.mini_player__x +
        ImprovedTube.mini_player__width -
        ImprovedTube.miniPlayer_resize_offset &&
...
ImprovedTube.miniPlayer_resize = function () {
  if (ImprovedTube.mini_player__cursor !== "") {
    ...
    window.addEventListener(
      "mousemove",
      ImprovedTube.miniPlayer_resizeMouseMove
    );
...
ImprovedTube.miniPlayer_resizeMouseMove = function (event) {
  if (ImprovedTube.mini_player__cursor === "n-resize") {
    ...
    ImprovedTube.mini_player__setSize(
      event.clientX - ImprovedTube.mini_player__x,
      ImprovedTube.mini_player__height
    );
  } else if (ImprovedTube.mini_player__cursor === "s-resize") {
    ImprovedTube.mini_player__setSize(
      ImprovedTube.mini_player__width,
      event.clientY - ImprovedTube.mini_player__y
    );
...
ImprovedTube.miniPlayer = function () {
  if (this.storage.mini_player === true) {
    ...
    this.elements.player.style.transform =
      "translate(" + 0 + "px, " + 0 + "px)";
...
ImprovedTube.playerRewindAndForwardButtons = function () {
  if (this.storage.player_rewind_and_forward_buttons === true) {
    ...
    this.createPlayerButton({
      ...
      onclick: function () {
        ImprovedTube.elements.player.seekTo(
          ImprovedTube.elements.player.getCurrentTime() + 5
        );
      },
...
ImprovedTube.playerIncreaseDecreaseSpeedButtons = function () {
  if (this.storage.player_increase_decrease_speed_buttons === true) {
    ...
    path1.setAttribute(
      "d",
      `M188.5,270.3c-24.4,28.1-23.2,71.7,2.6,98.6c14.4,15.1,33.7,22.6,52.9,22.6c18.8,0,37.5-7.2,51.8-21.5
    ...
    this.createPlayerButton({
      ...
      onclick: function () {
        const step =
          ImprovedTube.storage.player_custom_playback_speed_step || 0.25;
...
ImprovedTube.disableAutoDubbing = function () {
  ...
  function findOriginalAudioTrack(audioTracks) {
    ...
    function hasASR(track) {
      return (
        Array.isArray(track.captionTracks) &&
    ...
    const fallback =
      audioTracks.find((t) => t?.HB?.isDefault) || audioTracks[0];
...
ImprovedTube.jumpToKeyScene = function () {
  ImprovedTube.mostReplayed = function () {
    ...
    if (!data) return console.warn("Failed to extract ytInitialData.");
    ...
    const sortedMarkers = markers
      .slice()
      .sort((a, b) => a.decorationTimeMillis - b.decorationTimeMillis);
    const nextMarker =
      sortedMarkers.find((m) => m.decorationTimeMillis > currentMillis) ||
      sortedMarkers[0]; // fallback to first if none ahead
    const targetSeconds = nextMarker.decorationTimeMillis / 1000;

    player.currentTime = targetSeconds;
    player.play();
    console.log(
      `Jumped to Most Replayed @ ${Math.floor(targetSeconds / 60)}:${Math.floor(
        targetSeconds % 60
      )
        .toString()
        .padStart(2, "0")}`
    ...
    function extractYtInitialData() {
      ...
      for (let i = 0; i < scriptTags.length; i++) {
        if (DATA.ytInitialData) {
          var ytIData = DATA.ytInitialData;
        } else {
    ...
    function getMostReplayedMarkers(parsedJson) {
      const decorations =
        parsedJson?.["frameworkUpdates"]?.["entityBatchUpdate"]?.[
          "mutations"
        ]?.[0]?.["payload"]?.["macroMarkersListEntity"]?.["markersList"]?.[
          "markersDecoration"
  ...
  ImprovedTube.fetchDOMData2 = function () {
    try {
      ...
      try {
        DATA.title =
          document.getElementsByTagName("meta")?.title?.content || false;
        DATA.genre =
          document.querySelector("meta[itemprop=genre]")?.content || false;
        DATA.duration =
          document.querySelector("meta[itemprop=duration]")?.content || false;
      } catch {}
    }
    ...
    const waitForVideoTitle = setInterval(() => {
      ...
      if (title && title !== "YouTube") {
        ...
        console.log(
          "MOST REPLAYED: TITLE:" + ImprovedTube.videoTitle() + DATA.title
        );
        if (
          (DATA.title === ImprovedTube.videoTitle() ||
            DATA.title.replace(/\s{2,}/g, " ") === ImprovedTube.videoTitle()) &&
          ...
          (async function () {
            try {
              ...
              const htmlContent = await response.text();
              DATA.ytInitialData = htmlContent.match(
                /var ytInitialData = ({.*?});/s
              );
              if (DATA.ytInitialData) {
                ImprovedTube.mostReplayed();
              ...
              const o = Object.assign(document.createElement("div"), {
                innerText: "too few views",
              });
              const keySceneButton = document.querySelector(
                'button[data-tooltip="Key Scene"]'
              );
              if (keySceneButton) {
                keySceneButton.style.transition = "opacity 0.4s";
                keySceneButton.style.opacity = "0.3";
                setTimeout(() => {
                  keySceneButton.style.opacity = "0.8";
                }, 5000);
              }
              console.error(
                `Error: fetching from https://Youtube.com/watch?v=${DATA.videoID}`,
  ...
  if (currentPath.startsWith("/shorts/")) {
    ...
    if (videoId) {
      ...
      if (window.location.href !== newUrl) {
        console.log(
          `ImprovedTube: Redirecting Shorts to Watch: ${window.location.href} -> ${newUrl}`
        );
  ...
  if (this.storage.shorts_auto_scroll) {
    if (!ImprovedTube.shortsAutoScrollInterval) {
      ImprovedTube.shortsAutoScrollInterval = setInterval(() => {
        ...
        const video = activeRenderer
          ? activeRenderer.querySelector("video")
          : null;
