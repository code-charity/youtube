...
(function () {
    ...
    if (!window.ImprovedTube) return;

    const IT = window.ImprovedTube;
    const DBG = true;
    const log = (...a) => { try { if (DBG) console.log('[PlaylistCleaner]', ...a); } catch (e) {} };
    log('loaded. pathname=', location.pathname, 'search=', location.search);

    function isOnPlaylistPage() {
        try {
            const ok = IT.regex.playlist_id.test(location.search);
            log('isOnPlaylistPage?', ok);
            return ok;
        } catch (e) { log('isOnPlaylistPage error', e); return false; }
    }
    ...
    function getVideoIdFromRenderer(renderer) {
        ...
        if (a) {
            try { const m = a.href.match(IT.regex.video_id); if (m) return m[1]; } catch (e) {}
        }
        return null;
    }
    ...
    function saveLocalWatchHistory() {
        try {
            ...
            const data = IT.storage.watch_history || {};
    ...
    function resetWatchForVideo(videoId, renderer) {
        ...
        if (IT.storage.watch_history && IT.storage.watch_history[videoId]) {
            delete IT.storage.watch_history[videoId];
        ...
        try { IT.messages.send({ action: 'watched', type: 'remove', id: videoId }); } catch (e) {}
    }
    ...
    IT.playlistCleanerInitControls = function () { if (isOnPlaylistPage()) { log('initControls hook called'); createControls(); } };
    IT.playlistCleanerEnsureQuickButtons = function (node) { if (isOnPlaylistPage()) { ensureQuickButtons(node); } };

    // Fallback: in case the child handler didn’t run yet
    if (document.readyState !== 'loading') {
        log('DOMContentLoaded already passed, init now');
        IT.playlistCleanerInitControls(); attachQuickButtonsNow();
    } else {
        document.addEventListener('DOMContentLoaded', () => { log('DOMContentLoaded event, init now'); IT.playlistCleanerInitControls(); attachQuickButtonsNow(); });
    }
})();
