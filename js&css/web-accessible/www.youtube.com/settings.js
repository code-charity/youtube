...
ImprovedTube.improvedtubeYoutubeIcon = function () {
    ...
    button.addEventListener("mousedown", function (event) {
      if (ImprovedTube.storage.improvedtube_youtube_icon === "draggable") {
        ...
        function mousemove(event) {
          ...
          x = Math.min(
            Math.max(event.clientX - x2, 0),
            document.body.offsetWidth - 48
          );
          y = Math.min(
            Math.max(event.clientY - y2, 0),
  ...
  if (option === "header_left") {
    if (
      this.storage.header_position === "normal" &&
      this.elements.masthead.start
    ) {
...
ImprovedTube.deleteYoutubeCookies = function () {
  ...
  for (var i = 0, l = cookies.length; i < l; i++) {
    ...
    document.cookie =
      name +
      "=; domain=.youtube.com; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  }
