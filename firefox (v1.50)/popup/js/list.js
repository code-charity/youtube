...
function createList(obj = menu, options = { parent: document.querySelector('.main__container') }) {
  ...
  for (let key in obj)
    if (key != 'type' && key != 'classList' && key != 'disabled' && key != 'label' && key != 'label_description' && key != 'icon' && key != 'visibility' && obj[key].visibility != 'false')
...
function createListByPath() {
  ...
  if (path == '/')
    ...
    let by_key = searchByKey(new_menu, path_array[path_array.length - 1]);

    document.querySelector('.header__title').getMessage(by_key.label, true);
