...
function shortcut(elem, obj, key) {
  ...
  elem.addEventListener('click', function() {
    ...
    button_allow.getMessage('allow');

    deny.classList.add('dialog__surface__button');
    deny.getMessage('deny');
    ...
    function mousewheel(event) {
      ...
      value_elem.value = (it_keys.label && it_keys.label.length > 0 && it_keys.label[it_keys.label.length - 2] != '+' ? (it_keys.label + ' + ') : (it_keys.label || '')) +
        (event.deltaY > 0 ? 'scroll+' : 'scroll-');

      for (let i = 0, l = replace_text.length; i < l; i++)
        value_elem.value = value_elem.value.replace(replace_text[i], getMessage(replace_text[i]));

      it_keys.scroll = event.deltaY;
      it_keys.label = this.value;
    }
...
function slider(elem, obj, key) {
  let label = document.createElement('span'),
    ...
    input_status = document.createElement('span'),
    value = 0;
  ...
  input.oninput = function() {
    ...
    input_status.innerHTML = this.value;
  };
}
