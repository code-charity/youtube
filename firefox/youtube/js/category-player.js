...
function video_encode() {
    ...
    if (data != 'do_not_change' || !(gl && gl instanceof WebGLRenderingContext)) {
...
function video_autofullscreen() {
    ...
    var wait = setInterval(function() {
        if (document.getElementById('movie_player')) {
            document.getElementById('movie_player').toggleFullscreen();

            document.querySelector('#movie_player video').onended = function(event) {
                document.getElementById('movie_player').toggleFullscreen();
            };
        }
...
function mini_player() {
    ...
    var mini_player_data = {
        x: localStorage.getItem('improvedtube-miniplayer-x') || window.innerWidth - 350 - 32,
        y: localStorage.getItem('improvedtube-miniplayer-y') || window.innerHeight - 200 - 32
    };
    ...
    var wait = setInterval(function() {
        if (document.documentElement.getAttribute('page') == 'video' && (document.querySelector('#player-container') || document.querySelector('#player-api')) && document.querySelector('#movie_player video')) {
            ...
            var mini_player = {
                ...
                width: 350,
                height: 200,
                width_old: document.querySelector('.html5-video-player').offsetWidth,
                height_old: document.querySelector('.html5-video-player').offsetHeight,
                left_old: document.querySelector('.html5-video-player').offsetLeft,
                top_old: document.querySelector('.html5-video-player').offsetTop,
                offset: {
                    x: 0,
                    y: 0
                }
            ...
            window.addEventListener('resize', function() {
                mini_player.width_old = document.querySelector('.html5-video-player').offsetWidth;
                mini_player.height_old = document.querySelector('.html5-video-player').offsetHeight;
                mini_player.left_old = document.querySelector('.html5-video-player').offsetLeft;
                mini_player.top_old = document.querySelector('.html5-video-player').offsetTop;
            });
            ...
            window.addEventListener('scroll', function() {
                if (settings.mini_player_b == 'true' && document.documentElement.getAttribute('page') == 'video') {
                    if (window.scrollY > 500 && !document.documentElement.hasAttribute('mini-player')) {
                        ...
                        if (document.querySelector('#movie_player video').offsetLeft > window.innerWidth - 350 - 32) {
                            mini_player.x = window.innerWidth - 350 - 32;
                        }

                        if (document.querySelector('#movie_player video').offsetTop > window.innerHeight - 200 - 32) {
                            mini_player.y = window.innerHeight - 200 - 32;
                        }
                        ...
                        window.addEventListener('mousemove', function(event) {
                            if (mini_player.move) {
                                ...
                                if (event.clientX - mini_player.offset.x <= 32) {
                                    mini_player.x = 32;
                                } else if (event.clientX + (document.querySelector('#movie_player video').offsetWidth - mini_player.offset.x) >= window.innerWidth - 32) {
                                    mini_player.x = window.innerWidth - 32 - document.querySelector('#movie_player video').offsetWidth;
                                }

                                if (event.clientY - mini_player.offset.y <= 32) {
                                    mini_player.y = 32;
                                } else if (event.clientY + (document.querySelector('#movie_player video').offsetHeight - mini_player.offset.y) >= window.innerHeight - 32) {
                                    mini_player.y = window.innerHeight - 32 - document.querySelector('#movie_player video').offsetHeight;
                                }
