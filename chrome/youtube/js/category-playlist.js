...
function playlist_reverse_button() {
    setInterval(function() {
        if (!document.getElementById('improvedtube-playlist-reverse')) {
            if (
                ...
                if (document.documentElement.getAttribute('youtube-version') === 'old') {
                    ...
                    button.className = 'yt-uix-button yt-uix-button-size-default yt-uix-button-player-controls yt-uix-button-empty yt-uix-button-has-icon shuffle-playlist yt-uix-button-opacity yt-uix-tooltip';

                    if (globalReversePlaylist === true) {
                    ...
                    button.innerHTML = '<svg xmlns=//www.w3.org/2000/svg width=24 height=24 viewBox="0 0 24 24"><path d="M9 3L5 6.99h3V14h2V6.99h3L9 3zm7 14.01V10h-2v7.01h-3L15 21l4-3.99h-3z"></svg>';
                    ...
                    document.querySelector('ytd-watch-flexy ytd-playlist-panel-renderer #header-contents #playlist-actions ytd-menu-renderer #top-level-buttons').appendChild(button);
                }
            }
        } else if (document.getElementById('improvedtube-playlist-reverse').hasAttribute('hidden')) {
            document.getElementById('improvedtube-playlist-reverse').removeAttribute('hidden');
        }
    });
}

function playlist_reverse() {
    if (getUrlParams().hasOwnProperty('list') && globalReversePlaylist === true) {
        if (document.documentElement.getAttribute('youtube-version') === 'old') {
            let list = document.querySelector('#player-playlist .playlist-videos-list'),
