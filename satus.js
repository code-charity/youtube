...
satus.render = function (skeleton, container, skip, property) {
    if (skeleton.hasOwnProperty('component') && skip !== true) {
        ...
        if (skeleton.hasOwnProperty('storage') === false && property) {
            skeleton.storage = property;
        }
        ...
        component.change = function (value) {
            if (this.skeleton.storage) {
                satus.storage.set(this.skeleton.storage, value);
            }
        };
...
satus.components.select = function (skeleton) {
	...
	component.addEventListener('render', function () {
		var select = this.selectElement,
			value = satus.storage.get(this.skeleton.storage) || this.skeleton.options[0].value;
